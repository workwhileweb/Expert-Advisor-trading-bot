...
**Cách nhanh (Windows):** Chạy `install-ea.bat` — script sẽ tự dò tìm mọi thư mục MT5 (`%APPDATA%\MetaQuotes\Terminal\...` và `Program Files`), hiển thị danh sách rồi chép `bot.mq5` + `bot.ex5` theo lựa chọn.

```bash
install-ea.bat
```

**Hoặc làm thủ công:** Sao chép `bot.mq5` vào thư mục Experts của MetaTrader 5:
...
Sau đó biên dịch Expert Advisor trong MetaEditor (F7)
